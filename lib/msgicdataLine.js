	...
	{			
		...
		{			
			...
			else
			{
				...
				{
					fireEvent("click",{lineName:msg.lineName,xMark:msg.xMark,value:msg.value,selectData:that.dataset[msg.ID]});
					});
				lines.push(newLine);					
			}
		}
...
{				
	...
	this.setData=function(id,arr,xs,ys,color)
	{
		...
		.on("click",function(d,i)
			{					
				fireEvent("click",{ID:that.ID,lineName:that.lineName,xMark:that.xMarks[i],value:d});	
			});
		xScale=xs;
		yScale=ys;
		newData=arr;						
